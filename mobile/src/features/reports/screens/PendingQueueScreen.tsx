...
export function PendingQueueScreen() {
  ...
  const query = useInfiniteQuery({
    queryKey: ['pendingQueue', activeFilters],
    queryFn: ({ pageParam = 1 }: { pageParam?: number }) =>
      (reportApi as any).getPendingQueue({ ...activeFilters, page: pageParam }),
  ...
  const handleExport = async (format: 'pdf' | 'excel' | 'csv') => {
    setIsExporting(true);
    try {
      await downloadReport({ format, reportType: 'pending', ...activeFilters });
    } catch (err: any) {
  ...
  if (query.isLoading) return <LoadingScreen message="Loading pending queue..." />;
            ...
            {items.length > 0 && (
              <View style={{
                ...
                <Text style={[theme.typography.label, { color: theme.colors.warning, flex: 1 }]}>
                  {items.length} transaction{items.length !== 1 ? 's' : ''} awaiting approval
                </Text>
              </View>
            )}

            {query.isError && (
              <ErrorMessage message={parseApiError(query.error) ?? 'Failed to load queue'} onRetry={query.refetch} />
            )}
          </>
        )}
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={[theme.typography.caption, { color: theme.colors.textSecondary, marginTop: 2 }]}>
                  {item.collectionBranchId?.name ?? '—'} ({item.collectionBranchId?.code ?? '—'}) → {item.payoutBranchId?.name ?? '—'} ({item.payoutBranchId?.code ?? '—'})
                </Text>
                <Text style={[theme.typography.caption, { color: theme.colors.textSecondary, marginTop: 2 }]}>
                  By {item.createdBy?.name ?? '—'} · {fmtDate(item.createdAt)}
                </Text>
              </View>
              <View style={{ alignItems: 'flex-end' }}>
                ...
                {item.commissionAmount > 0 && (
                  <Text style={[theme.typography.caption, { color: theme.colors.textSecondary }]} allowFontScaling={false}>
                    Comm: {fmtAmt(item.commissionAmount)}
                  </Text>
                )}
              </View>
            </View>
